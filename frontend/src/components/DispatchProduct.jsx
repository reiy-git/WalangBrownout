...
export default function DispatchProduct({ products = [], recipients = defaultRecipients, onCancel, onApply, onViewAll }) {
  ...
  // Exit this form without saving a dispatch entry.
  const handleCancel = () => {
    if (onCancel) onCancel();
    navigate("/inventory");
  };

  // Take the user back to the full inventory list when they want to review all items.
  const handleViewAll = () => {
    if (onViewAll) onViewAll();
    navigate("/inventory");
  };

  // Save only valid dispatch quantities for the selected product.
