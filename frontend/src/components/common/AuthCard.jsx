    ...
    login({ email: username, password })
      .then((data) => {
        ...
        if (data.user) {
          ...
          localStorage.setItem("user_name", `${data.user.first_name || data.user.firstName || ''} ${data.user.last_name || data.user.lastName || ''}`.trim() || data.user.name || 'User');
        }
        if (onSuccess) {
          onSuccess(data.user);
        } else {
        ...
        <div className="card-body items-center text-center p-4 sm:p-5">
          <div className="mb-1 w-11 h-11 rounded-full bg-white shadow-sm flex items-center justify-center mx-auto">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L21 7V17L12 22L3 17V7L12 2Z" fill="url(#authBoxGrad)" stroke="#ffffff" strokeWidth="0.5" />
              ...
              <defs>
                <linearGradient id="authBoxGrad" x1="3" y1="2" x2="21" y2="22" gradientUnits="userSpaceOnUse">
          ...
          <form className="w-full flex flex-col gap-2.5 text-left" onSubmit={handleSubmit} noValidate>
            <div>
              <label className="text-[11px] font-medium text-violet-900/80 mb-0.5 block" htmlFor="email">
                Email
              </label>
              <div className="input input-sm w-full flex items-center gap-2 h-8 px-2.5 bg-white border border-violet-200/80 rounded-lg">
                ...
                <input
                  id="email"
                  type="email"
                  className="grow min-w-0 text-xs outline-none bg-transparent"
            ...
            {error && (
              <p className="text-[10px] text-red-500 text-center font-medium bg-red-50 p-1.5 rounded-md">
                {error}
              </p>
            )}
            ...
            <button
              ...
              className="btn btn-sm w-full bg-[#8b7fd6] hover:bg-[#7a6ec5] text-white border-0 rounded-lg text-xs font-semibold mt-2 h-8 min-h-[32px] shadow-xs transition-all"
            >
