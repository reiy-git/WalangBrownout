...
export default function ManagerEditUser() {
  ...
  const handleUpdate = () => {
    ...
    // Password fields optional on edit — only validate/update if user typed something
    ...
    const updated = users.map((u) =>
      String(u.id) === String(id)
        ? {
            ...
            email: email.trim(),
            role,
            position: u.position === u.role ? role : u.position, // keep custom position if set
            ...(password ? { password } : {}),
          }
        : u
    );
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full">
        ...
        <div className="navbar bg-[#e9d5ff] border-b border-[#ddd6fe] px-4 sm:px-6 shadow-xs flex justify-between items-center relative z-10">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-[#2e1065]">Edit/Update User</span>
          </div>
