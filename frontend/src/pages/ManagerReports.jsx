...
function isInPeriod(dateStr, period) {
  ...
  if (isNaN(d.getTime())) return true; // don't hide unparseable dates
  const now = new Date();
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full">
        ...
        <main className="max-w-6xl mx-auto px-4 sm:px-6 mt-8 relative z-10 w-full pb-12 flex-1 flex flex-col">
          ...
          <div className="flex flex-wrap justify-between items-center gap-3 mb-6">
            <h1 className="text-xl sm:text-2xl font-bold text-[#2e1065]">Report Management</h1>
            <div className="flex gap-2">
              <button
                onClick={() => navigate("/view-report")}
                className="btn btn-sm bg-[#8b7fd6] hover:bg-[#8b7fd6]/90 border-0 text-white font-medium px-4 rounded-lg text-xs"
              >
                View Report
              </button>
              <button
                onClick={() => navigate("/search-report")}
                className="btn btn-sm bg-white hover:bg-gray-50 border border-[#8b7fd6]/40 text-[#2e1065] font-medium px-4 rounded-lg text-xs"
              >
                🔍 Search Report
              </button>
            </div>
          ...
          <div className="flex gap-3 mb-6">
            {tabs.map((tab) => (
              <button
                ...
                className={`btn btn-sm px-5 rounded-lg text-xs font-semibold border-0 ${
                  activeTab === tab
                    ? 'bg-[#8b7fd6] text-white shadow-sm'
                    : 'bg-[#c4b5fd]/40 text-[#2e1065] hover:bg-[#c4b5fd]/60'
                }`}
              >
                {tab}
              </button>
